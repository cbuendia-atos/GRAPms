...
public class EsmoResponseFactory {
    ...
    public static AttributeSet buildErrorResponse(String issuer, String recipient, String smSessionId) {
        ...
        return new AttributeSet(id, TypeEnum.Request, issuer, recipient,
    ...
    public static AttributeSet buildFromMinEduResponse(MinEduResponse resp, String issuer, String recipient, String smSessionId) {
        ...
        if (resp.getResponse().toUpperCase().equals("SUCCESS")) {
            String id = UUID.randomUUID().toString();
            List<AttributeType> attributes = new ArrayList<>();
            String academicIdenitfier = resp.getInspectionResult().getAcademicId();
            String eduPersonAffiliation = TranslitarateUtils.convertGreektoLatin(resp.getInspectionResult().getDepartmentName());
            String primaryAffiliation = TranslitarateUtils.convertGreektoLatin(resp.getInspectionResult().getDepartmentName());
            String email = "";
            String schacExpiryDate = "";
            String mobile = "";
            String eduPersonPrincipalName = "";
            String eduPersonPrincipalNamePrior = "";
            String displayName = resp.getInspectionResult().getLatinFirstName() + " " + resp.getInspectionResult().getLatinLastName();
            String sn = resp.getInspectionResult().getLatinLastName();
            String givenName = resp.getInspectionResult().getLatinLastName();
            ...
            attributes.add(new AttributeType("https://software.internet2.edu/eduperson/internet2-mace-dir-eduperson-201602.html#eduPersonPrimaryAffiliation", "primaryAffiliation", "UTF-8", "en", false, (String[]) Arrays.asList(primaryAffiliation).toArray()));
